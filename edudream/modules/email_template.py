...
from edudream.modules.utils import send_email, decrypt_text, get_site_details
...
def parent_class_creation_email(classroom):
    ...
    translated_content = gettext(message)
...
def tutor_status_email(user):
    ...
    message = f"Dear {name}, <br><br>Your Tutor profile on EduDream is now active" \
              f"<br>Please login to your dashboard to: <br>- Complete or update your profile <br>- Add the subjects " \
              f"you want to teach<br>- Create your schedule<br><br>The earlier you do these, the earlier a student will reachout to you!"
    subject = "Account Activated"
    translated_content = gettext(message, "fr")
    translated_subject = gettext(subject, "fr")
    contents = render(None, 'default_template.html', context={'message': translated_content}).content.decode('utf-8')
    send_email(contents, email, translated_subject)
    return True
...
def send_class_reminder_email(user, classroom, minu):
...
def send_fund_pending_balance_email(user, classroom):
    first_name = user.first_name
    if not user.first_name:
...
def send_fund_main_balance_email(user, classroom):
    first_name = user.first_name
    if not user.first_name:
        first_name = "EduDream Tutor"
    email = user.email
